...
- **Breaking: `worker_id` is no longer a dimension on six metric instruments.** `taskq.lock.expires_in_seconds`, `taskq.heartbeat.misses`, `taskq.leader.election_attempts`, `taskq.leader.election_failures`, `taskq.cron.lock_contention` and the `taskq.heartbeat.consecutive_failures` gauge now emit a single undimensioned series each. **Dashboards, queries and alert rules that group or filter by `worker_id` on these metrics will break** — they return one series where they used to return one per worker. `worker_id` is a fresh UUID per worker *process*, so every deploy, restart and autoscale event minted new time series without bound; Azure Monitor counts each unique (metric, dimension key, dimension value) seen in 12 hours as an active series, caps a subscription at 50,000 per region, and throttles ingestion for *every* custom metric once the cap is passed, with no backfill of what was dropped. Per-worker attribution is unchanged on the channels where cardinality is free: `worker_id` is bound onto every log line via contextvars, and `taskq.worker_id` is a cron-fire span attribute. The `record_*` helpers still accept their `worker_id` argument — only the dimension is gone. (`taskq.cron.consecutive_failures` was subsequently relabeled too — see the next entry.)
- **Breaking: `taskq.cron.consecutive_failures` is relabeled `schedule_id` → `actor`, and the `actor` label is capped.** **Dashboards, queries and alert rules that group or filter this metric by `schedule_id` lose their series on upgrade** — the label is now the schedule row's `actor`, bounded to the first 100 distinct names a worker process sees (later names collapse onto the fixed `_other_` value). The relabel's original premise — that `actor` is the registered actor set — does not hold on this instrument: `create_schedule` accepts any string actor at creation time (validation is deferred to fire time by design, so schedules may reference actors that register later), and a dangling, misspelled or tenant-generated name fails every tick's planning loop, each failure emitting the raw string as a label value — one time series per distinct string, the exact OTLP cardinality failure the observability module guards against. Per-schedule attribution is unchanged on the channels where cardinality is free: the `cron fired` / `cron fire failed` / `cron schedule auto-disabled` log lines carry `schedule_id`, and the `cron fire` span carries `taskq.cron_schedule_id`. The per-actor balance is a sum over the actor's schedules and can carry permanent residue (a disabled, re-enabled or deleted schedule leaves its accumulated count behind; re-enabling resets the DB column with no metric delta), so `cron_schedules.consecutive_failures` and the logs are authoritative — alert on `taskq.cron.disabled_schedules > 0` (the shipped `rules.yaml` alert) rather than on this balance.
...
- `enqueue_batch_fast` now classifies a same-`(idempotency_scope, idempotency_key)` duplicate as the new typed `DuplicateIdempotencyKeyError` instead of a raw `asyncpg.UniqueViolationError` — the COPY path previously converted only the legacy-index violation (cross-scope reuse during the `01.00.03` window) to a typed error, while an in-batch duplicate or a race against the composite arbiter aborted the whole COPY as an untyped driver exception. The abort-the-whole-COPY semantics are unchanged (deliberate all-or-nothing bulk-import behavior; nothing is written); the fix is classification. The error names the offending pair when Postgres' violation detail reports it (best-effort parse, `None` on truncation), and the in-memory testing backend raises the same typed error with the pair named exactly (protocol parity; previously it raised a raw `UniqueViolationError` with the constraint name set). The non-fast paths never raise for this condition — their `ON CONFLICT` arbiter dedupes and returns the existing row — so no existing typed error fit; the new error follows pgqueuer's `DuplicateJobError` precedent.
- A NUL-bearing item no longer poisons a whole `enqueue_batch`/`enqueue_batch_fast` with an unattributed error — the batch build loops now annotate the rejection per item: `PayloadValidationError` naming the item index, the actor, and the field (`payload`/`metadata`/`tags`) that carried the NUL, with a synthetic sanitized `validation_errors` entry (`loc` = the field) — the same annotation contract the client layer's `_item_payload_error` gives pydantic failures. The batch still refuses atomically (nothing was written; the raise happens before any SQL is issued): the fix is attribution, not partial admission. The in-memory testing backend's batch path gets the same guard as a pre-insert preflight, fixing a parity break where it partially admitted items 0..k-1 before the NUL item raised.
