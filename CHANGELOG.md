...
- **`SubJobEnqueuer.enqueue(tags=[])` now suppresses tag inheritance instead of inheriting the parent's tags.** `tags=None` still means "no caller choice" and inherits (when `inherit_tags=True`, the default); an explicit empty list is now an affirmative "no tags". Previously `[]` was treated identically to `None`, so an actor passing `[]` to *suppress* tags silently got the parent's instead. **This is a silent behaviour change** — nothing raises, sub-jobs simply stop carrying the inherited tags, which also removes them from `JobFilter(tags=...)` results and from any `cancel_where` bulk cancel selecting on those tags. Only the `inherit_tags=True` plus non-empty-parent-tags case changes; every other row of the truth table is unchanged.
