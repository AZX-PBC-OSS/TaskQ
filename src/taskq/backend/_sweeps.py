...
:func:`reclaim_has_budget` is the in-process twin of this expression."""
...
def reclaim_has_budget(*, attempt: int, max_attempts: int, retry_kind: str) -> bool:
    """The in-process evaluation of :data:`_RECLAIM_HAS_BUDGET_SQL`.

    Every caller that reports on a reclaim it did not itself write —
    the in-memory twin's arm, and the isolate path's per-row tally —
    reads the question here rather than restating it, so a log line can
    never disagree with the row the statement actually wrote.
    """
    if retry_kind == "indefinite":
        return True
    return attempt < max_attempts and retry_kind != "non_retryable"
...
expired_outbox AS MATERIALIZED (
    ...
    --
    -- ORDER BY (occurred_at, id) pins the scan to the age-keyed outbox
    -- partial index (job_events_reclaim_age_idx), whose leading column
    -- makes this arm's age bound an Index Cond that STOPS at the
    -- boundary. The sibling id-keyed index (job_events_reclaim_idx)
    -- serves poll_reclaim_events' cursor tail instead; against that one
    -- the age bound can only be a post-scan Filter, so the arm would read
    -- every unconsumed outbox row on every tick. That population is
    -- exempt from ordinary retention and grows for the life of a
    -- deployment whose watch_reclaims consumer lags, so a Filter here
    -- makes the very arm that bounds the outbox unbounded in the
    -- outbox's own size. id and occurred_at are co-monotonic by the
    -- visibility-delay doctrine (constants.py
    -- RECLAIM_EVENT_VISIBILITY_DELAY), so this order is the same
    -- oldest-first drain as before, now servable without a top-N sort.
    SELECT id
    FROM "{schema}".job_events
...
-- The attempt number is resolved against the rows already stored rather
-- than taken from jobs.attempt. A job can legitimately reach this sweep
-- with its current attempt already spent: a transient retry, a crash
-- reclaim and an admin retry all re-pend the row WITHOUT advancing the
-- counter (the monotonic-attempt rule — dispatch claims at attempt + 1,
-- so resetting would revisit spent numbers), and each leaves behind the
-- job_attempts row for the execution that did happen. Writing this
-- sweep's row at that spent number collides on job_attempts_pkey, and
-- because the whole batch is one statement in one transaction the
-- collision rolls back every other job swept in the same call — so one
-- such row strands every overdue sibling, on every tick, forever
-- (dispatch also skips rows past schedule_to_close, so nothing else can
-- ever resolve them).
-- GREATEST over the stored maximum keeps the number monotonic for a job
-- whose counter is ahead of its attempt rows (a deadline reached before
-- the first dispatch ever wrote one), and the LEAST cap is the smallint
-- max_attempts discipline: at the ceiling the sweep reuses the top
-- number rather than overflowing, and ON CONFLICT then keeps the stored
-- outcome of the execution that really ran.
INSERT INTO "{schema}".job_attempts
...
SELECT a.job_id,
       LEAST(
           GREATEST(
               a.attempt,
               COALESCE(
                   (SELECT max(p.attempt) FROM "{schema}".job_attempts p
                     WHERE p.job_id = a.job_id) + 1,
                   a.attempt
               )
           ),
           32767
...
_SWEEP_IDLE_KEYED_BUCKETS_SQL = _SWEEP_IDLE_KEYED_BUCKETS_BODY.replace(
    "{no_consumed_quota_unqualified}", no_consumed_quota_sql()
).replace("{no_consumed_quota_b}", no_consumed_quota_sql("b"))
...
-- Two-step candidate selection. An aggregate in HAVING can only be
-- applied after grouping, so a single grouped scan must read and
-- aggregate EVERY keyed row in the table before the LIMIT can discard
-- anything — per-tick cost that grows with the whole keyed population.
-- The inner `aged` step instead seeks the keyed partial index with a
-- row-level bound, which is an Index Cond that stops at the horizon: a
-- bucket cannot qualify unless at least one of its rows is older than
-- the horizon, so this is a lossless pre-filter on bucket NAMES.
--
-- The verdict is then taken per candidate name over that bucket's FULL
-- row set, un-filtered. That re-read is the load-bearing half: max() is
-- not distributive over a row filter, so folding the bound into the
-- grouped scan would hide exactly the rows that disqualify a bucket. A
-- live bursty tenant — configured for more concurrency than it uses, so
-- its idle high-index slots stay stale while one fresh row proves it
-- live — would have its fresh row filtered out, its max computed over
-- the stale remainder, and the bucket would be selected and deleted out
-- from under a live workload.
...
stale AS MATERIALIZED (
    SELECT a.bucket_name
    FROM aged a
    CROSS JOIN LATERAL (
        SELECT max(s0.last_used_at) AS newest, bool_and(s0.keyed) AS all_keyed
        FROM "{schema}".reservation_slots s0
        WHERE s0.bucket_name = a.bucket_name
    ) g
    WHERE g.newest < statement_timestamp() - $1::interval
      AND g.all_keyed
    ORDER BY g.newest, a.bucket_name
    LIMIT $2
),
