    ...
    FROM (
        ...
        FROM (
            SELECT j.actor,
                   ...
                   -- otherwise. COALESCE covers the no-actor-config shape,
                   -- where the join finds nothing and the label is all
                   -- the row has.
                   CASE WHEN j.status = 'pending' AND j.started_at IS NOT NULL
                        THEN coalesce(ac.queue, j.queue)
                        ELSE j.queue
                   END AS queue,
