...
async def _await_factory_bounded(payload_factory: str, work: Awaitable[object]) -> object:
    """Await one phase of a payload factory under :data:`_FACTORY_TIMEOUT_S`.
    ...
    """
    try:
        return await asyncio.wait_for(work, timeout=_FACTORY_TIMEOUT_S)
    except TimeoutError as exc:
        reason = str(exc)
        if reason:
            raise TimeoutError(f"cron payload factory {payload_factory!r}: {reason}") from exc
        raise TimeoutError(
            f"cron payload factory {payload_factory!r} timed out after {_FACTORY_TIMEOUT_S:g}s"
        ) from exc
...
async def resolve_payload(
    ...
    *raw_metadata*.  Returns ``{}`` if neither is set.
    ...
    if payload_factory is not None:
        ...
        result: object = await _await_factory_bounded(payload_factory, asyncio.to_thread(factory))
        if inspect.iscoroutine(result):
            # A coroutine-returning factory keeps loop affinity: its body
            # runs here, on the loop, under the same deadline.
            result = await _await_factory_bounded(payload_factory, result)
