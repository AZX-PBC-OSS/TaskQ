...
class InMemoryBackend:
    ...
    async def retry_job(self, job_id: JobId) -> bool:
        ...
        # numbers, so no attempt-row writer can revisit a spent epoch's
        # key. At the smallint bound the ceiling cannot rise further and
        # the retry is refused — the row stays terminal — rather than
        # re-pending a job the next claim could only overflow.
