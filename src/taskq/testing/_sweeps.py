...
async def _reclaim_expired_locks(
    ...
    *,
    ...
    for job_id, row in list(self._jobs.items()):
        ...
        # The twin of the backend's RECLAIM_RETRYABLE_PREDICATE: an
        # ``indefinite`` job's budget is its schedule_to_close deadline, not
        # max_attempts, so a crash hands it back for another attempt whatever
        # its attempt number and the deadline sweep is what ends it.
        if row.retry_kind != "non_retryable" and (
            row.retry_kind == "indefinite" or row.attempt < row.max_attempts
        ):
