...
self-reinforcing. The fleet looked healthy the whole time:
...
again, recycle the worker (or let the orchestrator do it);
...
A timed-out sweep records its duration but **no row sample**, because the batch was
...
match set in bounded committed batches, with `event_writer_batch_size` driving
...
failing, with connection loss or constraint trouble) is different: one enqueue
...
tick's transaction. The whole tick rolls back, so the schedules stay due
...
- **Caller-supplied values** (payloads, metadata, labels, keyed-ref names)
  ...
  mis-classify it as transient infrastructure failure, and the job would retry
