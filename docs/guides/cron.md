...
funded budget)` (1.125 seconds at defaults). A smaller leftover funds no call;
...
- a factory the tick **could not fund a grant for** (the budget is spent, or the
  ...
  auto-disable in lockstep (the lockstep auto-disable bug), and granting the leftover as a micro-grant marched
  them just as surely behind a *slow-successful* monopolizer (the manufactured-strike bug): a factory called
  under a ~0.08s grant it cannot fit is cut by `wait_for` into a plain timeout:
...
- `"allof"` still owes the **later pass**. The tick that fires the last earlier-pass match
  ...
  exactly once, either by its pre-scheduled job or by the schedule's own later tick, never both.
...
on its granted deadline, so the factory **ran**), the cron loop:
...
- A singleton actor with an active job suppresses the slot. Watch for the
  `singleton-collision` log event with `detection_path="cron_tick_preflight"`;
