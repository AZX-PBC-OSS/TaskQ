...
| `max_sse_connections` | `int \| None` | `None` (`TASKQ_PROGRESS_MAX_SSE_CONNECTIONS`, default 50) | Maximum concurrent progress streams this process will serve; further connections get HTTP 429. Each stream holds a Redis pubsub subscription and an asyncio task for as long as the client stays connected. See [admin-ui.md: SSE connection limits](admin-ui.md#sse-connection-limits). |
| `resolve_pg_pool` | `Callable[[Request], asyncpg.Pool] \| None` | `None` | Per-request resolver for the pool, used instead of `pg_pool` when given. A host whose pool is replaced while it runs (`taskq ui serve` rebuilds it on a credential rotation) resolves the live one per request. A FastAPI dependency: it may declare `request: Request`. |
| `resolve_redis_client` | `Callable[[Request], Any] \| None` | `None` | Per-request resolver for the Redis client, used instead of `redis_client` when given — same credential-rotation pattern as `resolve_pg_pool`. A FastAPI dependency: it may declare `request: Request`. |
