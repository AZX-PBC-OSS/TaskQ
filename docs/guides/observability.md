...
| `terminal-write-retry` | warning | `terminal_write_retry` | `job_id`, `actor`, `write`, `attempt`, `max_attempts`, `retry_in_ms`, `infra_error_class`, `infra_error_message` | A terminal write hit an infrastructure error (connection reset, pool-acquire timeout) and is about to be retried — one line per retried attempt |
