...
| `taskq.jobs.reclaimed` | `1` | `actor`, `disposition` | Running jobs reclaimed by the expired-locks sweep (the holder broke its liveness promise — lease expiry or heartbeat timeout). `disposition` is the code-fixed enum `repended` (attempts remained, the row went back to pending on its retry curve), `crashed` (budget exhausted, terminal), `cancelled` (a cancel request was in-flight when the holder died — the honest terminal label). The per-actor crash split beside the sweep-name total `taskq.maintenance_leader.sweep_rows`; recorded once per (actor, disposition) pair after the sweep's transaction, by the Postgres sweep and the in-memory twin alike. | unconditional |
