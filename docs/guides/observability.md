...
| `taskq.jobs.attempt_failures` | `1` | `actor`, `error_type`, `retryable` | Attempts that ended in an actor failure, recorded at the failure handler once per handled exception. `retryable="true"`: the classifier rescheduled the attempt for another try; `"false"`: the failure is terminal. `error_type` is the exception class name. `TaskQRetryRateHigh` reads the `retryable="true"` share. | yes |
...
| `taskq.notify.received` | n/a | n/a | NOTIFY callbacks delivered from asyncpg on the listener connection (all channels' payloads, matching and not). | unconditional |
| `taskq.notify.reconnects` | n/a | n/a | Successful listener reconnects: PG connection churn on the NOTIFY path. Each increment is one dedicated connection the listener had to re-establish; a sustained rate is the NOTIFY-connection-failure story (an unstable direct DSN, see [troubleshooting.md](troubleshooting.md#5-notify-connection-failures)), not individual job trouble. | unconditional |
| `taskq.notify.cancel_received` | n/a | n/a | Cancel NOTIFY callbacks delivered to this worker (payload parsed, `type="cancel"`, `worker_id` matching this worker) and turned into cancel wakeups. | unconditional |
| `taskq.notify.payload_parse_failed` | `1` | n/a | NOTIFY payloads discarded because they did not parse to the expected JSON envelope. Dropping is the correct outcome for a channel TaskQ does not exclusively own, but a rising rate is not a quiet channel: it is a foreign writer or a schema change on the channel publishing bodies the envelope cannot decode. | unconditional |
| `taskq.cancel.notify_sent` | n/a | `schema` | `pg_notify` calls fired for running-job cancel requests (the fast path beside the heartbeat poll, which remains authoritative). | unconditional |
...
| `taskq.dispatch.corrupt_rows` | `1` | `queue`, `column` | Claimed rows whose jsonb columns refused to decode to the row contract, counted in the dispatch round's per-row decode path. `queue` is capped (like `taskq.dispatch.failures`); `column` is a closed set: `payload`, `metadata`, `progress_state`, `result` (or `unknown`, when the failing column cannot be determined). Each counted row is terminally failed in the same round with `error_class='CorruptJobDataError'`, so a poisoned row is visible here instead of as a producer loop that keeps dying on the unexpected-failure backstop: any non-zero rate is a producer writing garbage rows (schema drift, an interop writer). | yes |
| `taskq.enqueue.dedups` | `1` | `dedup_reason` | Enqueue dedup hits (an enqueue returned an existing row instead of writing one). `dedup_reason` is a bounded enum of three: `unique_for`, `idempotency_key`, or `zombie_ack_lost` (the first attempt committed its row but died before its acknowledgement; the retry's INSERT hit the committed row, and the identity stays pinned to that zombie's job id instead of erroring for work that succeeded). The per-hit log lines are budget-bounded at batch scale; this counter is the rate signal that survives the bound. | yes |
...
| `taskq.maintenance_leader.sweep_unexpected_errors` | `1` | `sweep_name` | Sweep batches aborted by an error outside the deadline family (a constraint violation, a connection reset, any fault that is not a statement cancel). The deadline family lands on `taskq.maintenance_leader.sweep_timeouts`; everything else that aborts a prune-family batch lands here, because the sweep's row counters and success stamps only move on success, so a drain stopped by this class reads healthy on every other plane. One increment per aborted batch; the first aborted batch ends the sweep attempt, so the rate tracks the prune retry ladder. `TaskQSweepUnexpectedErrors` reads it. | yes |
| `taskq.worker.loop_unexpected_errors_total` | `1` | `loop` | Unexpected (non-transient) errors tolerated by a long-lived loop's backstop (`UnexpectedLoopErrorGuard`): the leader maintenance loops, the worker producer loop, and the progress flush loop. The guard re-raises after its consecutive-failure budget, so every increment is an error the loop chose to survive - per its own description, "anything above zero warrants investigation: either PG produced a shape the transient set should learn, or the loop has a bug." Prescribe the alert at > 0, not at a rate. | yes |
...
| `taskq.maintenance_leader.sweep_last_success_seconds` | `s` | `sweep_name` | Unix timestamp of each sweep's last successful call. `time() - this value` is sweep staleness; a value that never moves while the process runs is a stalled sweep. Both operands of that difference are wall clock (the stamp is taken from `time.time()`, Prometheus's `time()` is scrape-time wall clock), so a backward clock step mutes any staleness alert built on it for the catch-up window; see the clock-domain caveat in the [TaskQPromotionStalled runbook](runbooks.md#taskqpromotionstalled). |
...
They are not recorded as dimensions.
...
| `TaskQSweepTimeouts`, `TaskQSweepDegraded`, `TaskQSweepUnexpectedErrors`, `TaskQLeaderLockContention` | maintenance sweep load, no worker knob | [TaskQSweepTimeouts](runbooks.md#taskqsweeptimeouts), [TaskQSweepDegraded](runbooks.md#taskqsweepdegraded), [TaskQSweepUnexpectedErrors](runbooks.md#taskqsweepunexpectederrors), [TaskQLeaderLockContention](runbooks.md#taskqleaderlockcontention) |
...
- The unconditional counters (`taskq.cancellation.requested`,
  ...
  `taskq.jobs.reclaimed`, `taskq.backpressure.capacity_refresh_failures`)
