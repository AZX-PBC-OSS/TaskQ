...
- **No leader:** ensure at least one worker is running. Failover is bounded by `leader_lease + heartbeat_interval` plus one round trip (50s at defaults) for a leader that went silent, and by about one round trip for one that exited cleanly (the resign is the shutdown's first act, before the drain). For one that kept winning the row but could not open its leader connections the bound is the same pair **plus one failing cycle's connection attempts** (each bounded by `TASKQ_RELOAD_FACTORY_TIMEOUT`, 30s at defaults, so roughly 80s, not 50s; the bound rests on the hand-back, not the lapse, because sub-lease re-wins keep refreshing `expires_at` so the row never lapses while the pod keeps winning). See `leader-resigned-unassumable` below.
...
- **Increase tolerance:** set `TASKQ_MAX_HEARTBEAT_FAILURES` higher (e.g. `5`) to absorb transient blips. Keep `lock_lease >= max(heartbeat_interval, heartbeat_command_timeout) + (max_heartbeat_failures + 1) * (heartbeat_interval + heartbeat_command_timeout)`, raising `TASKQ_MAX_HEARTBEAT_FAILURES` or `TASKQ_HEARTBEAT_COMMAND_TIMEOUT` raises the floor with it.
...
- **Multiple schemas:** each schema gets its own `maintenance_leader` row. Verify `TASKQ_SCHEMA_NAME` is consistent across all workers. Failover is bounded by `leader_lease + heartbeat_interval` plus one round trip (50s at defaults) for a leader that went silent, and by about one round trip for one that exited cleanly (the resign is the shutdown's first act, before the drain); if slower, check that `leader_conn` uses a direct DSN and the watchdog health check (every 5s) is not blocked.
...
| Timing invariant violation | `lock_lease < max(heartbeat_interval, heartbeat_command_timeout) + (max_heartbeat_failures + 1) * (heartbeat_interval + heartbeat_command_timeout)`, or `cancellation + cleanup >= termination_grace - 5.0` or `>= lock_lease`. |
...
- **Timing invariant violations:** adjust settings so `lock_lease >= max(heartbeat_interval, heartbeat_command_timeout) + (max_heartbeat_failures + 1) * (heartbeat_interval + heartbeat_command_timeout)` and `cancellation + cleanup < termination_grace - 5.0` and `< lock_lease`.
