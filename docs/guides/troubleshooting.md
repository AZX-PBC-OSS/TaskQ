...
- **`leader-resigned-unassumable` in the logs:** that pod won elections but could not open the dedicated leader connections (connection-count pressure, a credential-factory outage), so it handed the lease row back after one `leader_lease` window and kept handing re-wins back until it recovered. A peer should have taken over within `leader_lease + heartbeat_interval`; if the WARN keeps repeating on one pod, that pod's connection budget or credential factory is the fault to fix — the fleet is leading from elsewhere.
