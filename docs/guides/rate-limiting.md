...
4. A rate-limited job is rescheduled (not failed or retried): its status is `scheduled` with `scheduled_at` set past the denial's backoff, and it is promoted back to `pending` when that time arrives. There is no `snoozed` value in `job_status` — query for `scheduled` rows with a non-zero `rate_limit_blocked_count`.
