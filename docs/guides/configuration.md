...
| `TASKQ_TIMESCALEDB_HYPERTABLES` | `bool` | `false` | Opt in to TimescaleDB hypertables for `jobs_archive`, `job_attempts_archive` and `job_events`, with retention policies derived from `TASKQ_ARCHIVE_RETENTION_PERIOD` and `TASKQ_EVENT_RETENTION_PERIOD`. With the flag off (the default), setup issues zero new statements and vanilla Postgres is fully supported. With the flag on, `taskq migrate up` requires the server to offer and load the `timescaledb` extension and fails loudly naming this setting when it does not. See [timescaledb.md](timescaledb.md). | n/a |
