...
| **Async-native** | Yes (asyncio + asyncpg); the full stack speaks async natively |
...
| **DI engine** | Yes (scoped providers), which resolve dependencies at dispatch time |
...
- [:material-tune: Configuration](guides/configuration.md): All `TASKQ_*` env vars, `.env` cascade, pool sizing, watchdog, PgBouncer
- [:material-clipboard-check: Operations & Adoption](guides/ops.md): The one-page ops guide: timeouts, sizing math, fan-out patterns, footguns
- [:material-tools: Troubleshooting](guides/troubleshooting.md): Problem-solution reference for stuck jobs, crashes, NOTIFY failures, and more
