...
async def _advance_and_promote(backend: Backend, target_time: datetime) -> int:
    ...
    # PG: server-side clock_timestamp() controls promotion; force
    # scheduled_at to the past so the sweep promotes the job.
    import asyncpg
...
async def test_snooze_cycle_refunds_attempt_round_trip(
    ...
    result = await backend_pair.mark_snoozed(
        job_id, wid, timedelta(seconds=30), attempt=row.attempt, claim_epoch=row.claim_epoch
    )
...
async def test_snooze_past_deadline_transitions_to_failed(
    ...
    result = await backend_pair.mark_snoozed(
        ...
        claim_epoch=epoch_row.claim_epoch,
    )
...
async def test_mark_interrupted_releases_pending_at_zero_hold(
    ...
    epoch past the one the interrupted handler holds."""
    job_id, wid = await _enqueue_dispatch_any(backend_pair)
    ...
    row = await backend_pair.get(job_id)
    assert row is not None
    _assert_job_row(row, status="pending", attempt=1)
    assert row.interrupt_count == 1

    # No deferral floor on a zero hold: the row is claimable NOW, without
    # advancing either backend's clock; at a FRESH epoch (no refund).
...
async def test_mark_interrupted_holds_the_release_until_the_process_is_gone(
    ...
    not be claimable until the releasing process is provably gone), the
    spent attempt standing; the row promotes and re-dispatches once the
    hold elapses."""
    job_id, wid = await _enqueue_dispatch_any(backend_pair)
    ...
    outcome = await backend_pair.mark_interrupted(
        job_id, wid, attempt=row.attempt, claim_epoch=row.claim_epoch, hold=timedelta(seconds=30)
    )
...
async def test_mark_interrupted_operator_cancel_in_flight_wins(
    ...
    assert row.attempt == 1, "a declined release moves nothing"
...
async def test_mark_interrupted_fenced_out_by_a_stale_epoch_or_wrong_worker(
    ...
    outcome = await backend_pair.mark_interrupted(
        job_id, wid, attempt=row.attempt, claim_epoch=row.claim_epoch - 1, hold=timedelta(0)
    )
    assert outcome == "noop"

    # Wrong worker.
    outcome = await backend_pair.mark_interrupted(
        job_id, new_uuid(), attempt=row.attempt, claim_epoch=row.claim_epoch, hold=timedelta(0)
    )
    assert outcome == "noop"
...
async def test_mark_succeeded_transitions_row_and_emits_attempt(
    ...
    epoch_row = await backend_pair.get(job_id)
    assert epoch_row is not None
    result = await backend_pair.mark_succeeded(
        job_id, wid, {"ok": True}, attempt=epoch_row.attempt, claim_epoch=epoch_row.claim_epoch
    )
...
async def test_mark_cancelled_transitions_row_and_emits_attempt(
    ...
    epoch_row = await backend_pair.get(job_id)
    assert epoch_row is not None
    result = await backend_pair.mark_cancelled(
        job_id, wid, attempt=epoch_row.attempt, claim_epoch=epoch_row.claim_epoch
    )
    assert result is True
    ...
    # second call is idempotent - returns False, no second attempt row
    # (the stale epoch presentation is the worker's own no-op shape)
    result2 = await backend_pair.mark_cancelled(
        job_id, wid, attempt=epoch_row.attempt, claim_epoch=epoch_row.claim_epoch
    )
    assert result2 is False
    attempts2 = await backend_pair.get_attempts(job_id)
...
async def test_result_size_bytes_set_on_mark_succeeded(backend_pair: Backend) -> None:
    ...
    epoch_row = await backend_pair.get(job_id)
    assert epoch_row is not None
    ok = await backend_pair.mark_succeeded(
        job_id, wid, result_payload, attempt=epoch_row.attempt, claim_epoch=epoch_row.claim_epoch
    )
    assert ok is True
...
async def test_result_size_bytes_none_when_result_is_none(backend_pair: Backend) -> None:
    ...
    await backend_pair.mark_succeeded(
        job_id, wid, None, attempt=epoch_row.attempt, claim_epoch=epoch_row.claim_epoch
    )
...
async def test_mark_succeeded_wrong_worker_returns_false(backend_pair: Backend) -> None:
    ...
    assert epoch_row is not None
    result = await backend_pair.mark_succeeded(
        job_id, wrong_wid, None, attempt=epoch_row.attempt, claim_epoch=epoch_row.claim_epoch
    )
...
async def test_indefinite_tier_fail_retry_succeed(backend_pair: Backend) -> None:
    ...
    row_after = await backend_pair.mark_failed_or_retry(
        job_id, wid, error_info, retry_delay, attempt=row.attempt, claim_epoch=row.claim_epoch
    )
