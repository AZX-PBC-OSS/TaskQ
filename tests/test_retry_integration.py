...
async def _promote_scheduled_to_running(
    ...
    A real claim advances the claim epoch beside the attempt; the epoch
    rides the promoted attempt so the next terminal write's fence sees
    what dispatch would have stamped.
    """
    await conn.execute(
        ...
        worker_id,
        job_id,
        # A separate bind: the smallint attempt column and the bigint
        # epoch column cannot share one parameter (asyncpg deduces the
        # type per parameter, and the two deduce differently).
        attempt,
    )
...
async def test_transient_retry_succeeds_after_retries(
    ...
    result = await backend.mark_succeeded(job_id, worker_id, {"ok": True}, attempt=3, claim_epoch=3)
...
async def test_transient_exhaustion(
    ...
    row_2 = await backend.mark_failed_or_retry(
        job_id, worker_id, error_info, None, attempt=2, claim_epoch=2
    )
...
async def test_cancel_skips_classifier() -> None:
    ...
    cancelled = await backend.mark_cancelled(job_id, worker_id, attempt=1, claim_epoch=1)
...
async def test_indefinite_retry_polling_pattern(
    ...
    result = await backend.mark_succeeded(
        job_id, worker_id, {"ok": True}, attempt=11, claim_epoch=11
    )
    assert result is True
...
async def test_indefinite_retry_deadline_enforcement(
    ...
    row_1 = await backend.mark_failed_or_retry(
        job_id, worker_id, error_info, decision_1.retry_delay, attempt=1, claim_epoch=1
    )
    assert row_1.status == "scheduled"
    ...
    row_2 = await backend.mark_failed_or_retry(
        job_id, worker_id, error_info, decision_2.retry_delay, attempt=2, claim_epoch=2
    )
    assert row_2.status == "failed"
