...
async def _interrupt_database(dsn: str) -> int:
    ...
    xdist worker's database, so the scope must come from the terminating
    connection itself: a name passed in from elsewhere would let a typo or a
    reused fixture terminate a sibling worker's backends.
    """
