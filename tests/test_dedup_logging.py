...
def _dedup_points_by_reason(
    ...
    return {str(dict(p.attributes or {}).get("dedup_reason")): int(p.value) for p in points}


async def test_idempotency_dedup_hit_counts_on_enqueue_dedups(
...
async def test_unique_for_dedup_hit_counts_on_enqueue_dedups(
    otel_reader: InMemoryMetricReader,
) -> None:
...
async def test_batch_dedup_stampede_counts_every_hit_despite_warning_suppression(
    ...
    from taskq.backend._enqueue import (
        _DEDUP_WARN_PER_HIT_LIMIT,  # pyright: ignore[reportPrivateUsage]  # Why: the bound under attack is the production constant; redefining it here would let the pin drift from the budget that actually runs.
    )
