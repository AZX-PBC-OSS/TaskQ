...
async def test_operator_cancel_racing_a_failure_retry(
    ...
    for i in range(60):
        ...
        async def _retry(
            ...
            await asyncio.sleep(_head)
            # The fenced-out retry reads back as the handler's no-op. The
            # claim epoch is the seeded row's own (the seeding helper
            # stamps 1 beside attempt 1): the epoch fence must not be
            # what refuses this write, the race is the cancel stamp's.
            with contextlib.suppress(WorkerOwnershipMismatch):
                await backend.mark_failed_or_retry(  # type: ignore[arg-type]
                    _job_id, _worker_id, _BOOM, _ONE_S, attempt=1, claim_epoch=1
                )
    ...
    for i in range(10):
        ...
        with contextlib.suppress(WorkerOwnershipMismatch):
            await backend.mark_failed_or_retry(
                job_id, worker_id, _BOOM, _ONE_S, attempt=1, claim_epoch=1
            )
...
async def test_cancel_origin_truth_table_differential_pg_vs_memory(
    ...
    async def pg_case(
        ...
        try:
            row = await app.backend.mark_failed_or_retry(
                job_id, worker_id, _BOOM, delay, attempt=1, claim_epoch=1
            )
    ...
    async def memory_case_async(
        ...
        try:
            row = await backend.mark_failed_or_retry(
                jid, worker_id, _BOOM, delay, attempt=1, claim_epoch=1
            )
        except WorkerOwnershipMismatch:
            return ("mismatch", None)
        return _outcome(row)
...
async def test_retry_after_cancel_terminal_is_a_clean_noop(
    ...
    assert await backend.mark_cancelled(job_id, worker_id, attempt=1, claim_epoch=1)
...
async def test_retry_without_the_attempt_epoch_never_lands(
    ...
    with pytest.raises(WorkerOwnershipMismatch):
        await backend.mark_failed_or_retry(
            job_id, worker_id, _BOOM, _ONE_S, attempt=None, claim_epoch=None
        )
    rows = await backend.list_jobs(JobFilter())
    mine = [row for row in rows if row.id == job_id]
