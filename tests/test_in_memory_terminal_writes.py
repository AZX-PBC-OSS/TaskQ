...
class TestTerminalWriteIdempotency:
    ...
    async def test_mark_succeeded_single_attempt_and_event(self) -> None:
        ...
        r1 = await backend.mark_succeeded(
            job_id, wid, result={"ok": True}, attempt=1, claim_epoch=1
        )
        assert r1 is True

        r2 = await backend.mark_succeeded(job_id, wid, result=None, attempt=1, claim_epoch=1)
        assert r2 is False
...
class TestOwnershipMismatch:
    ...
    async def test_bool_returning_wrong_worker_returns_false(self) -> None:
        ...
        assert await backend.mark_cancelled(job_id, wrong_worker, attempt=1, claim_epoch=1) is False

        assert (
            await backend.mark_snoozed(
                job_id, wrong_worker, timedelta(seconds=30), attempt=1, claim_epoch=1
            )
            == "noop"
        )
...
class TestSingleAttemptRowPerTransition:
    ...
    async def test_mark_failed_terminal_attempt_and_event(self) -> None:
        ...
        await backend.mark_failed_or_retry(job_id, wid, error_info, None, attempt=1, claim_epoch=1)
    ...
    async def test_mark_snoozed_attempt_and_event(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        await backend.mark_snoozed(job_id, wid, timedelta(seconds=30), attempt=1, claim_epoch=1)
...
class TestMarkSnoozedIdempotencyAndMetadataMerge:
    ...
    async def test_metadata_update_none_preserves_existing(self) -> None:
        ...
        await backend.mark_snoozed(
            job_id2, wid2, delay, metadata_update=None, attempt=1, claim_epoch=1
        )
    ...
    async def test_snoozed_attempt_outcome(self) -> None:
        ...
        job_id, wid = await _enqueue_and_dispatch(backend)
        delay = timedelta(seconds=30)
        await backend.mark_snoozed(job_id, wid, delay, attempt=1, claim_epoch=1)
...
def _bump_to_attempt(backend: InMemoryBackend, job_id: JobId, attempt: int) -> None:
    """Move a running, worker-owned row to a later attempt epoch.

    The same-worker reclaim/redispatch shape: the row stays ``running``
    and owned by the SAME worker, but the attempt epoch advanced (sweep
    re-pended it, dispatch re-claimed it at attempt + 1). A handler
    suspended since the earlier epoch must not be able to terminalise
    the new one.
    """
    row = backend._jobs[job_id]  # type: ignore[reportPrivateUsage]  # Why: test-only private access
    backend._jobs[job_id] = replace(  # type: ignore[reportPrivateUsage]  # Why: test-only private access
        row,
        attempt=attempt,
        # A real reclaim + redispatch also advances the claim epoch (the
        # twin's claim bumps it every claim); the simulated redispatch
        # must move both or the fence under test is not the shipped one.
        claim_epoch=row.claim_epoch + 1,
    )


class TestAttemptEpochFencing:
    """A terminal write fenced on the attempt epoch no-ops a stale attempt.
    ...
    async def test_mark_succeeded_stale_attempt_no_ops(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        _bump_to_attempt(backend, job_id, 2)

        assert await backend.mark_succeeded(job_id, wid, None, attempt=1, claim_epoch=1) is False, (
            "a stale attempt's success write must not terminate the redispatched attempt"
        )
        row = await backend.get(job_id)
        assert row is not None
        assert row.status == "running" and row.attempt == 2

    async def test_mark_succeeded_without_attempt_no_ops(self) -> None:
        """A write that cannot present ANY attempt epoch must not land.

        This is the red-team pin's exact shape
        (``mark_succeeded(job_id, worker_id, None)``): a caller with no
        attempt-identity snapshot is indistinguishable from the stale
        handler and must be fenced out, never allowed to terminate a row
        whose epoch it cannot prove.
        """
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
    ...
    async def test_mark_succeeded_live_attempt_lands(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        _bump_to_attempt(backend, job_id, 2)

        assert await backend.mark_succeeded(job_id, wid, None, attempt=2, claim_epoch=2) is True
        row = await backend.get(job_id)
        assert row is not None
        assert row.status == "succeeded"

    async def test_mark_cancelled_stale_attempt_no_ops(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        _bump_to_attempt(backend, job_id, 2)
    ...
    async def test_mark_cancelled_without_attempt_no_ops(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)

        assert await backend.mark_cancelled(job_id, wid) is False

    async def test_mark_cancelled_live_attempt_lands(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        _bump_to_attempt(backend, job_id, 2)

        assert await backend.mark_cancelled(job_id, wid, attempt=2, claim_epoch=2) is True

    async def test_mark_snoozed_stale_attempt_no_ops(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        _bump_to_attempt(backend, job_id, 2)
    ...
    async def test_mark_snoozed_without_attempt_no_ops(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
    ...
    async def test_mark_snoozed_live_attempt_lands(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        _bump_to_attempt(backend, job_id, 2)

        result = await backend.mark_snoozed(
            job_id, wid, timedelta(seconds=30), attempt=2, claim_epoch=2
        )
        assert result == "scheduled"

    async def test_mark_retry_after_stale_attempt_no_ops(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        _bump_to_attempt(backend, job_id, 2)

        result = await backend.mark_retry_after(
            job_id, wid, timedelta(seconds=10), attempt=1, claim_epoch=1
        )
        assert result == "noop"
        row = await backend.get(job_id)
        assert row is not None
    ...
    async def test_mark_retry_after_live_attempt_lands(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        _bump_to_attempt(backend, job_id, 2)

        result = await backend.mark_retry_after(
            job_id, wid, timedelta(seconds=10), consume_budget=False, attempt=2, claim_epoch=2
        )
        assert result == "scheduled"

    async def test_mark_failed_or_retry_stale_attempt_raises_mismatch(self) -> None:
        """The failure family mirrors PG: a fenced-out write raises
        ``WorkerOwnershipMismatch`` (rowcount 0 → the same raise the worker
        fence produces), which ``safe_mark_failed_or_retry`` converts to
        the None the handler treats as a no-op."""
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
        _bump_to_attempt(backend, job_id, 2)
    ...
    async def test_mark_failed_or_retry_without_attempt_raises_mismatch(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend)
    ...
    async def test_mark_failed_or_retry_live_attempt_lands(self) -> None:
        backend = _make_backend()
        job_id, wid = await _enqueue_and_dispatch(backend, max_attempts=3)
        _bump_to_attempt(backend, job_id, 2)
        ...
        result = await backend.mark_failed_or_retry(
            job_id, wid, error_info, timedelta(seconds=10), attempt=2, claim_epoch=2
        )
...
class TestWrongWorkerIdHandling:
    ...
    async def test_mark_succeeded_wrong_worker_false(self) -> None:
        ...
        job_id, _wid = await _enqueue_and_dispatch(backend)
        assert (
            await backend.mark_succeeded(job_id, new_uuid(), None, attempt=1, claim_epoch=1)
            is False
        )
    ...
    async def test_mark_cancelled_wrong_worker_false(self) -> None:
        backend = _make_backend()
        job_id, _wid = await _enqueue_and_dispatch(backend)
        assert await backend.mark_cancelled(job_id, new_uuid(), attempt=1, claim_epoch=1) is False
    ...
    async def test_mark_snoozed_wrong_worker_false(self) -> None:
        ...
        assert (
            await backend.mark_snoozed(
                job_id, new_uuid(), timedelta(seconds=30), attempt=1, claim_epoch=1
            )
            == "noop"
        )
    ...
    async def test_mark_failed_or_retry_wrong_worker_raises(self) -> None:
        ...
        with pytest.raises(WorkerOwnershipMismatch):
            await backend.mark_failed_or_retry(
                job_id, new_uuid(), error_info, None, attempt=1, claim_epoch=1
            )
...
class TestSnoozePastDeadline:
    ...
    async def test_in_memory_snooze_within_deadline_succeeds(self) -> None:
        ...
        result = await backend.mark_snoozed(
            job_id, wid, timedelta(seconds=5), attempt=1, claim_epoch=1
        )
        assert result == "scheduled"
...
class TestDenialNeverConsumesRetryBudget:
    ...
    async def test_in_memory_denial_at_budget_ceiling_reschedules(self, outcome: str) -> None:
        ...
        row = backend._jobs[job_id]  # type: ignore[reportPrivateUsage]  # Why: test-only private access
        backend._jobs[job_id] = replace(row, attempt=3, claim_epoch=3)  # type: ignore[reportPrivateUsage]  # Why: test-only private access
    ...
    async def test_in_memory_repeated_denials_never_exhaust_budget(self) -> None:
        ...
        for expected in range(1, 8):
            ...
            backend._jobs[job_id] = replace(
                ...
                # Each simulated cycle is a reclaim + redispatch, so the
                # claim epoch advances with the attempt (twin's claim).
                claim_epoch=row.claim_epoch + 1,
            )
...
class TestMarkRetryAfterMaxAttemptsFails:
    ...
    async def test_in_memory_mark_retry_after_max_attempts_fails(self) -> None:
        ...
        result = await backend.mark_retry_after(
            job_id, wid, timedelta(seconds=10), consume_budget=True, attempt=3, claim_epoch=3
        )
...
class TestMarkRetryAfterIndefiniteTierIgnoresMaxAttempts:
    ...
    async def test_in_memory_mark_retry_after_indefinite_tier_ignores_max_attempts(self) -> None:
        ...
        row = backend._jobs[job_id]  # type: ignore[reportPrivateUsage]  # Why: test-only private access
        backend._jobs[job_id] = replace(row, attempt=5, claim_epoch=5)  # type: ignore[reportPrivateUsage]  # Why: test-only private access

        result = await backend.mark_retry_after(
            job_id, wid, timedelta(seconds=10), consume_budget=True, attempt=5, claim_epoch=5
        )
        assert result == "scheduled"
...
class TestMarkRetryAfterPastDeadlineFails:
    ...
    async def test_retry_after_past_deadline_consumer_side_fails(self) -> None:
        ...
        result = await backend.mark_retry_after(
            job_id, wid, timedelta(seconds=30), consume_budget=True, attempt=1, claim_epoch=1
        )
...
class TestMarkSucceededResultExpiryFallback:
    ...
    async def test_cleared_stored_ttl_falls_back_to_literal_at_completion(self) -> None:
        ...
        ok = await backend.mark_succeeded(
            job_id, wid, {"ok": True}, fallback_result_ttl=ttl, attempt=1, claim_epoch=1
        )
        assert ok is True
    ...
    async def test_no_stored_no_fallback_keeps_enqueue_pinned_value(self) -> None:
        ...
        ok = await backend.mark_succeeded(job_id, wid, {"ok": True}, attempt=1, claim_epoch=1)
        assert ok is True
