...
async def test_health_request_times_out_after_connect(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    The connect phase succeeds (mock returns a reader/writer immediately),
    but readline never produces data, so the per-request asyncio.timeout fires.
    Uses mocks instead of a real Unix socket server to avoid a Python 3.12
    epoll FD cleanup issue where the event loop blocks indefinitely after
    ...
    async def _never_returns() -> bytes:
        await asyncio.sleep(10)
        return b""

    mock_reader = AsyncMock()
    mock_reader.readline = _never_returns
