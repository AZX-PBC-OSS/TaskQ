...
no later seq ever recovers it. The terminal exemption is the client mirror
of the stream's own terminal-delivery rule.
"""
...
const timeline = {
    appendChild(entry) {
        ...
        metas.push(meta ? meta.textContent : "");
    },
};
...
advance(1000);
advance(32000);
process.stdout.write(JSON.stringify({ log, metas }));
"""
...
async def test_the_client_cursor_renders_the_durable_terminal_behind_the_wire_inflated_seq(
    ...
    page. On the pre-fix client the poll body was skipped, both drivers
    stopped, and the page froze on attempt 1's stale 50%.
    """
    ...
    metas = outcome["metas"]
    # Five attempt-1 deltas rendered live (the real envelopes carry ts, so
    # each meta is "<percent> · <clock>"), then the durable terminal entry.
    assert [m.split(" ")[0] for m in metas[:5]] == ["10%", "20%", "30%", "40%", "50%"]
    assert metas[-1] == "25%", (
        f"the page must end on the durable terminal's state, not stale attempt-1 progress: {metas}"
    )
