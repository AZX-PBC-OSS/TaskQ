...
* Both backends' terminal modules are walked. The Postgres backend supplies
  the nine production call paths (the completeness tripwire fails if the
  net loses or gains one); the in-memory twin ships no SQL today, its
  writes are the Python mirrors the differential corpus pins, so the same
  net walks it too and arity-checks any statement that ever appears there
  instead of the module being exempted silently.
...
def _statement_attrs(node: ast.expr, aliases: dict[str, ast.expr]) -> frozenset[str] | None:
    ...
    mark_* statement reference."""
    if isinstance(node, ast.Attribute):
        if node.attr.startswith("mark_"):
            return frozenset({node.attr})
        return None
    if isinstance(node, ast.IfExp):
        body = _statement_attrs(node.body, aliases)
        orelse = _statement_attrs(node.orelse, aliases)
        if body is None or orelse is None:
            return None
        return body | orelse
    if isinstance(node, ast.Name) and node.id in aliases:
        return _statement_attrs(aliases[node.id], aliases)
    return None
...
def test_backend_call_sites_match_placeholder_arity(rendered: dict[str, str]) -> None:
    """Every mark_* fetch in either backend's terminal module binds exactly
    the statement's placeholder count, and the Postgres side's net still
    sees all nine statements (a walk that lost a call site would pass
    vacuously otherwise)."""
    sites = [
        site
        for path in _TERMINAL_MODULES
