...
entry points do and runs the production ``worker_main`` entry against
the shared actor registry. Pure glue: no signal handling (``worker_main``
owns the loop and the post-drain SIGTERM window), no business logic, no
...
if __name__ == "__main__":
    ...
    try:
        sys.exit(worker_main(settings, actor_registry=ACTORS))
