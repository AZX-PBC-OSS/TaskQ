"""Livelock/starvation pins for the progress-recovery driver.

The driver under attack is the ASSEMBLED client (#487's
``static/realtime.js``: the fingerprint/ETag render machinery, the
conditional poll, the single cursor-gated acceptProgress) carrying the
#478 recovery semantics forward: the empty-snapshot skip, the cross-source
seq cursor, and stream recovery as close-on-error plus
browser-reconnect-with-polling-standby. The salvage adapts the #486 pins
to that shape. Four candidate loop shapes come with the territory, and
each gets a deterministic pin driven under Node with virtual timers (same
harness family as ``test_realtime_js_mode_probe.py``; assertions are on
issued requests, DOM writes and teardown, never on source text):
...
1. SPIN: a skip must not change the loop's cadence. The recovery loop
   ...
   never arms the poller hotter, and no interval leaks. The conditional
   poll rides the same cadence: the tick that learns the cursor answers
   304 from then on.
...
The render record is the DOM-write log: the assembled client renders
update-in-place (the entry is appended once, every later tick patches
nodes whose value changed), so a percent render IS a ``width`` write and
an idle page writes nothing at all.
...
# the badge mode, the section attributes, fetch bodies per URL (the
# server's ETag contract modelled once: an If-None-Match equal to the
# row's progress_seq is answered 304 with no body), a manually driven
# EventSource whose handlers the scenario fires, and an advance() that
# runs due timers in order. stats is the pin's liveness counter set.
_HARNESS = r"""
const fs = require("fs");
...
const stats = {
    pollTicks: 0,
    stateFetches: [],   // {at, inm} per /state fetch (inm: the If-None-Match or null)
    modeFetches: 0,
    writes: [],         // every DOM write: {at, kind: "append"|"width"|"text", value}
    sseOpens: 0,
    sseCloses: 0,
    intervalsCreated: 0,
    liveIntervals: 0,
...
function makeNode(tag) {
    ...
    Object.defineProperty(node, "textContent", {
        ...
        set(v) {
            text = v;
            stats.writes.push({ at: now, kind: "text", value: v });
        },
    });
    return node;
}
const timeline = {
    children: [],
    appendChild(entry) {
        stats.writes.push({ at: now, kind: "append", value: entry._name });
        ...
        Object.defineProperty(bar.style, "width", {
            ...
            set(v) {
                width = v;
                stats.writes.push({ at: now, kind: "width", value: v });
            },
        });
    },
};
...
function ruleFor(url) {
    for (const rule of scenario.fetchRules || []) {
        if (url.endsWith(rule.suffix)) return rule;
    }
    return null;
}

// Promise-like chain: the first .then receives the response object
// ({status, json}), every later .then receives the previous callback's
// result; catch is a no-op anywhere (the module's poll chain swallows
// errors).
const valChain = (getV) => {
    const p = {
        then(f) {
...
const respChain = (resp) => {
    const p = {
        then(f) {
            let v;
            try { v = f(resp); } catch (e) { v = undefined; }
            return valChain(() => v);
        },
        catch() { return p; },
    };
...
global.fetch = (url, opts) => {
    const inm = opts && opts.headers ? opts.headers["If-None-Match"] : undefined;
    if (url.endsWith("/sse/mode")) {
        stats.modeFetches += 1;
        const rule = ruleFor(url);
        return respChain({ status: 200, json: () => (rule ? rule.body : {}) });
    }
    stats.stateFetches.push({ at: now, inm: inm === undefined ? null : inm });
    const rule = ruleFor(url);
    if (rule && rule.notModified) {
        return respChain({ status: 304, json: () => null });
    }
    const body = rule ? rule.body : {};
    // The endpoint's ETag contract: the row's progress_seq doubles as the
    // ETag, and a request whose If-None-Match matches the row is answered
    // 304 with no body at all.
    if (
        inm !== undefined &&
        body.progress_seq !== undefined &&
        inm === `"${body.progress_seq}"`
    ) {
        return respChain({ status: 304, json: () => null });
    }
    return respChain({ status: 200, json: () => body });
};
...
def _percents(stats: dict[str, Any]) -> list[str]:
    """The rendered percentages, in order: the assembled client renders
    update-in-place, so a percent render is a bar-width write."""
    return [w["value"] for w in stats["writes"] if w["kind"] == "width"]
...
# the "empty snapshot" the skip exists for.
...
def test_pin_sustained_empty_polls_wake_at_exactly_the_poll_interval() -> None:
    ...
    leaks intervals breaks the count. The conditional poll rides the same
    cadence: the first tick learns the cursor, every tick after is
    ...
    assert stats["stateFetches"][0]["inm"] is None  # the first tick learns the cursor
    assert all(f["inm"] == '"3"' for f in stats["stateFetches"][1:]), (
        "every tick after the first carries the cursor as If-None-Match"
    )
    assert stats["intervalsCreated"] == 1  # no re-arm, no leak
    assert stats["liveIntervals"] == 1
    assert stats["writes"] == []  # every empty snapshot skipped, no render churn
...
def test_pin_sustained_empty_sse_frames_stay_on_the_stream() -> None:
    ...
    assert stats["writes"] == []  # every empty frame skipped
...
def test_pin_event_after_an_empty_skip_renders_at_push_latency() -> None:
    ...
    percents = _percents(stats)
    assert len(percents) == 1
    render_at = next(w["at"] for w in stats["writes"] if w["kind"] == "width")
    assert render_at == 500  # push latency, not a poll period
    assert percents == ["10%"]
...
def test_pin_reconnect_catch_up_is_not_dropped_and_the_wake_is_not_lost() -> None:
    ...
    # the poll reconciled the outage gap: the 90 rendered from the poll
    # tick (at 1300), before the reconnect at t=4000
    poll_90 = [w for w in stats["writes"] if w["kind"] == "width" and w["value"] == "90%"]
    assert poll_90 and poll_90[0]["at"] < 4000
    assert percents[-1] == "95%"  # the post-recovery wake was NOT lost
    # the poller stood down at the reconnect (t=4000): no fetch after it
    assert stats["stateFetches"] and max(f["at"] for f in stats["stateFetches"]) < 4000
...
def test_pin_minutes_of_empties_and_flaps_never_exit_the_recovery_machinery() -> None:
    ...
    assert stats["writes"] == []  # nothing rendered, nothing spun
...
def test_pin_terminal_sse_frame_tears_down_and_late_frames_are_inert() -> None:
    ...
    assert "50%" in _percents(stats)  # the frame before the terminal rendered
...
def test_mutation_catchup_drop_is_caught_by_the_starvation_pin() -> None:
    ...
    assert "90%" not in _percents(stats)
