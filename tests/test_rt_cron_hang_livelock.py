...
async def slow_monopolizer_factory() -> dict[str, object]:
    ...
    The review shape: a slow-but-successful monopolizer never
...
class TestHungFactoryBatchCannotLivelockTheTick:
    ...
    leader cadence, no strike, because their factories never ran (:
    ...
    async def test_a_batch_of_hung_factories_still_commits_the_strike_defers_the_rest_and_fires_the_peer(
        ...
        for row in deferred:
            ...
            assert due < row["next_fire_at"] < next_slot, (
                ...
                "staying at the owed slot is the strike path (no "
...
class TestSlowSuccessfulMonopolizer:
    """The review: a monopolizer that SUCCEEDS within its grant every
